...
void mtl_k0005_init_c6_0_0_func(
  ...
  constexpr float tmp4 = 0.4;
  const float tmp5 = (tmp2 * tmp4);
  constexpr float tmp6 = 0.2;
  const float tmp7 = (tmp5 + tmp6);
  const float tmp8 = (tmp3 * tmp4);
  const float tmp9 = (tmp8 + tmp6);
  S0 tmp6526(root_addr);
  constexpr int32_t tmp6595 = 0;
  S0_ch tmp6528 = tmp6526.children(tmp6595);
  S1 tmp6529 = tmp6528.get0(runtime_, mem_alloc_);
  constexpr int32_t tmp6577 = 8191;
  const int32_t tmp6578 = (tmp1 & tmp6577);
  S1_ch tmp6532 = tmp6529.children(tmp6578);
  device float* tmp6533 = tmp6532.get0(runtime_, mem_alloc_).val;
  *tmp6533 = tmp7;
  device float* tmp6543 = tmp6532.get1(runtime_, mem_alloc_).val;
  *tmp6543 = tmp9;
  constexpr float tmp15 = 0.0;
  S4 tmp6549 = tmp6528.get1(runtime_, mem_alloc_);
  S4_ch tmp6552 = tmp6549.children(tmp6578);
  device float* tmp6553 = tmp6552.get0(runtime_, mem_alloc_).val;
  *tmp6553 = tmp15;
  constexpr float tmp18 = -1.0;
  device float* tmp6563 = tmp6552.get1(runtime_, mem_alloc_).val;
  *tmp6563 = tmp18;
  constexpr float tmp21 = 1.0;
  S12 tmp6569 = tmp6528.get3(runtime_, mem_alloc_);
  S12_ch tmp6572 = tmp6569.children(tmp6578);
  device float* tmp6573 = tmp6572.get0(runtime_, mem_alloc_).val;
  *tmp6573 = tmp21;
}

}  // namespace
kernel void mtl_k0005_init_c6_0_0(
  ...
  for (int ii = begin_; ii < end_; ii += ugrid_size_) {
    mtl_k0005_init_c6_0_0_func(root_addr, global_tmps_addr, runtime_addr, print_assert_addr, ii);
  }
}
